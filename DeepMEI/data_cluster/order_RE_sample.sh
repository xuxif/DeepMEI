...
if [[ $step -eq 1 ]]
then
	samtools view -T $REF -H $bam_file |head -n 100  >head_$ran_num.sam
else
...
ls |sed -n '/mapClipL.sam$/p'|while read record;do cat <(cat ../head_$ran_num.sam) <(grep -v "^@" $record |grep -v "^$" |perl -F'\t' -alne 'print "$F[5]\t$_";'|perl -npe "s/(\d+)S.*?\t/\1\t/" |perl -F'\t' -alne '$start=$F[4]-$F[0];print "$start\t$_";'|sort -k1,1n |cut -f3-) > ../split_softclipped_sort_$ran_num/$record;rm $record ;done &

ls |sed -n '/mapClipR.sam$/p'|while read record ;do  cat <(cat ../head_$ran_num.sam) <(grep -v "^@" $record |grep -v "^$" |sort -k4,4n ) > ../split_softclipped_sort_$ran_num/$record;rm $record ;done &

ls |sed -n '/mapRef.sam$/p'|while read record ;do cat <(cat ../head_$ran_num.sam) <(grep -v "^@" $record |grep -v "^$" |sort -k4,4n ) > ../split_softclipped_sort_$ran_num/$record;rm $record ;done &
