...
ran_num=$1
parallel=$4
echo -ne "Processing raw filtering 1/4"
...
ls |grep "mapRef"|cut -d'_' -f2|xargs -I{} -P $parallel bash ../indel_size_filter.sh $ran_num $REF $bam_file {}  | perl -F'\t' -alne 'if(($F[1]==0 and $F[3]==0) or ($F[1]==0 and ($F[3] <10 or $F[3]>40)) or ($F[3]==0 and ($F[1] <10 or $F[1]>40)) or ($F[1]==0 and $F[2]>$F[4]) or ($F[3]==0 and $F[2]<$F[4]) ){ print "$_"}'|perl -npe "s/[:\-]/\t/g"  >filter_largeIndel.bed &
...
../../DeepMEI_script/SE-MEI/extractSoftclipped -l 15 <(samtools view -h $bam_file  -T $REF `cat tmp_map_clip.txt |perl -F'\t' -alne '$start=$F[2]-50;$end=$F[2]+50;print "$F[1]\t$start\t$end";'|bedtools merge |perl -npe "s/\t/:/;s/\t/-/;s/\n/ /"|perl -npe "s/$/\n/"`|perl ../select_ori_read.pl ) |samtools import  /dev/stdin 2>/dev/null |samtools fasta  2>/dev/null |cut -d'|' -f1,4,7,9  |perl -F'\|' -alne 'if($_=~/^>/){ if($F[2]=~/S$/){$F[2]="right";}else {$F[2]="left";} $name="$F[0]|$F[1]|$F[3]|$F[2]"; }else {print $seq="$name\t$_\t".length($_);}'|sort -k1,1 -k3,3nr |uniq |perl -F'\t' -alne 'if($last=~/$F[1]/) {} else { print "$F[0]\n$F[1]";} $last=$F[1];' >tmp_clip.fa
...
#date
ls rm_tmp_clip_split*|xargs -P $parallel -I{} bash  ../repeatmask.sh {} 
