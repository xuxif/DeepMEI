...
ls |grep "mapRef"|cut -d'_' -f2|xargs -P 20 -I{} perl ../check_polyAT.pl {} |uniq  >filter_polyN.bed
...
ls |grep "mapRef"|cut -d'_' -f2|xargs -I{} -P 20 bash ../indel_size_filter.sh $ran_num $REF $bam_file {} 2>/dev/null | perl -F'\t' -alne 'if(($F[1]==0 and $F[3]==0) or ($F[1]==0 and ($F[3] <10 or $F[3]>40)) or ($F[3]==0 and ($F[1] <10 or $F[1]>40)) or ($F[1]==0 and $F[2]>$F[4]) or ($F[3]==0 and $F[2]<$F[4]) ){ print "$_"}'|perl -npe "s/[:\-]/\t/g"  >filter_largeIndel.bed &
...
ls tmp_discordant_split_* |xargs -P 20 -I{} bash ../get_dis_mate_fasta.sh $bam_file $REF {}
...
#date
ls rm_tmp_clip_split*|xargs -P 20 -I{} bash  ../repeatmask.sh {} 2>&1 >/dev/null
