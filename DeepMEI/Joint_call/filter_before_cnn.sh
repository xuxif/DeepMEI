...
done |perl ../BP_z_trans.pl | perl -npe "s/\t/:/;s/\t/\-/;" |xargs -n 1 -P 20 -I {} perl ../indel_len_filter.pl $bam_file $REF {} >filter_indel.bed &

ls |grep "mapRef"|cut -d'_' -f2|xargs -n 1 -I{} -P 20 bash ../indel_size_filter.sh $ran_num $REF $bam_file {} 2>/dev/null | perl -F'\t' -alne 'if(($F[1]==0 and $F[3]==0) or ($F[1]==0 and ($F[3] <10 or $F[3]>40)) or ($F[3]==0 and ($F[1] <10 or $F[1]>40)) or ($F[1]==0 and $F[2]>$F[4]) or ($F[3]==0 and $F[2]<$F[4]) ){ print "$_"}'|perl -npe "s/[:\-]/\t/g"  >filter_largeIndel.bed &
...
cat tmp_clip.fa.out |perl -npe "s/HG002_//"|grep -E "SINE\/Alu|LINE\/L1|Retroposon\/SVA"|perl -npe "s/^ +//;s/ +/\t/g" |cut -f5,10,11|perl -npe "s/_.*?\t/\t/;s/:/\t/;s/\-/\t/"|perl -F'\t' -alne 'if($F[0]=~/soft/) { @region=split(/\|/,$F[0]);$start=$region[2]-1;$end=$region[2]+1;print "$region[1]\t$start\t$end\t$F[1]\t$F[2]";} else {print $_;}' >filter_rm.bed
