...
cat  tmp_clip.fa|perl -npe "s/(>.*)\n/\1\t/" |perl -F'\t' -alne '$i++;print "$i\t$_";if($i==20){$i=0;}' |while read id info;do echo "$info" |perl -npe "s/[ \t]+/\n/" >>tmp_clip_split_$id ;done

ls tmp_clip_split*|xargs -P 20 -I{} bash  ../repeatmask.sh {} 2>&1 >/dev/null
