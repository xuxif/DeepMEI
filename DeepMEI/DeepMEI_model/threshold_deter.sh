...
seq $start $step $end|while read score;do pp=`cat $input|perl -npe "s/$/\t$score/"|perl -F'\t' -alne 'if($F[4]>=$F[6]) { print "$_";}'|wc -l `;tp=`bedtools intersect -b /DeepAlu/final_vcf/true_giab.bed -a <(cat $input|perl -npe "s/$/\t$score/"|perl -F'\t' -alne 'if($F[4]>=$F[6]) { print "$_";}' )  -wa |wc -l `;precision=`echo "scale=5;$tp/$pp"|bc`;recall=`echo "scale=5;$tp/2000"|bc`;f1=`echo "scale=5;2*$precision*$recall/($precision+$recall)"|bc`;echo -e "$score\t$pp\t$tp\t$precision\t$recall\t$f1";done
