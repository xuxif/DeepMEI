...
try:
    opts, args = getopt.getopt(sys.argv[1:],"i:s:o:t:r:",["input_gt=","split_softclipped_sort=","output=","reference="])
    print(opts)
except getopt.GetoptError:
  print('python model_test.py  -i <input_gt> -o <output_predict_file> -sss <split_softclipped_sort> -ref <reference>')
  sys.exit(2)
for opt, arg in opts:
  if opt == '-h':
    print('python model_test.py  -i <input_gt> -o <output_predict_file> -sss <split_softclipped_sort> -ref <reference>')
...
def run_test(hparams,load_model,load_model_ckpt,genotype_file,predict_file, use_existing_data=True,seed=1):
  random.seed(seed)
...
#    print(features.shape)
    ...
    for predict_ori_each,record_use_ori_each,feature_ori_each in zip(predict_ori,record_use,features):
        if predict_ori_each[0]<0.6:
          features_pad.append(feature_ori_each)
...
#    print(features_pad.shape)
    record_use_select=np.array(record_use_select)
...
#      test_dataset = test_dataset.batch(batch_size=128)
      print('new image generate finished:'+str(t))
...
run_test(hparams,'last','cp-0088.ckpt',genotype_file,predict_file,True)
