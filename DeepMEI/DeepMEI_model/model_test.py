...
try:
    opts, args = getopt.getopt(sys.argv[1:],"i:s:o:q:r:",["input_gt=","split_softclipped_sort=","output=","quick_model=","reference="])
    print(opts)
...
#      if padding <0 :
#          print(features_pad.shape)
#          features_pad=np.delete(features_pad,-1,2)
#          print(features_pad.shape)
#          features_pad=np.insert(features_pad,0,[[0]*6]*100,2)
#          print(features_pad.shape)
#      elif padding >0:
#          features_pad=np.delete(features_pad,0,2)
#          features_pad=np.insert(features_pad,-1,[[0]*6]*100,2)
#      else:
#          features_pad=features_select
      t=len(features_pad)
...
#      print('new image generate finished')
      predict.append(model.predict(test_dataset))
#      print('predict finished')
#      print(predict[-1])
      record_use_total.append(record_use_select)
#  predict=np.array(predict)
#  print(predict.shape)
#  record_use_total=np.array(record_use_total)
  with open(predict_file, 'w', encoding='utf-8') as res_f:
    for record_pad_predict,record_pad_info in zip(predict,record_use_total):
      for predict_info,genotype_info in zip(record_pad_predict,record_pad_info):
        chr,pos,gt,data_type,me=genotype_info.strip().split('\t')
        j=np.argmax(predict_info)
        max_t=max(predict_info)
        if (predict_info[1]+predict_info[2])>=1 or max_t>=1:
          phred_score_find=87
        else:
        ...
        res_f.write(genotype_info.strip()+'\t'+str(j)+'\t'+str(phred_score_max)+'\t'+str(phred_score_find)+'\t'+str(me)+'\t'+str(predict_info[0])+'\t'+str(predict_info[1])+'\t'+str(predict_info[2])+'\n')
