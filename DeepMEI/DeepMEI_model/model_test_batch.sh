...
then
	...
	base=/
	ln -s ${base}/root/DeepAlu ${base}/DeepAlu
	export PATH=${base}/DeepAlu/anaconda3/bin:$PATH
	if [[ ! -n "$REF" ]]; then
		REF=${base}/DeepAlu/DeepAlu_model/reference/hs37d5.fa
	...
	then
		...
		REF=${base}/DeepAlu/DeepAlu_model/reference/Homo_sapiens_assembly38.fasta
...
if [ ! -n "$ME_REF" ]; then
	ME_REF=${base}/DeepAlu/DeepAlu_model/reference/ME.fa
...
#	/DeepAlu/DeepAlu_model/samtools-1.10/samtools view -T $REF $bam_file  -o $new_bam -@ 20
#	bam_file=$new_bam
#	samtools index $bam_file -@ 20
#fi	
input_bai=`echo $bam_file|perl -npe "s/\.bam/\.bai/"`
if [[ ! -f "${bam_file}.bai" ]]  && [[ ! -f $input_bai ]]
then
	samtools index $bam_file -@ 20
fi
...
if [ ! -n "$input_gt" ]; then
	...
	cd ${base}/DeepAlu/DeepAlu_script/
	bash discover_bam_to_candidate.sh $bam_file $ME_REF $output $REF
	input_gt=${base}/DeepAlu/DeepAlu_script/vsoft_pos/${output}_extract_vsoft.bed
else
...
input_gt=${base}/DeepAlu/DeepAlu_script/vsoft_pos/${ran_num}_vsoft.bed


echo "Starting DeepAlu prediction ......"
cd ${base}/DeepAlu/DeepAlu_model

threshold_model=`cat depth_threshold.txt|grep "^$depth	"|cut -f2`
#cp  $input_gt input_gt.txt
#python model_test.py       -i $input_gt -o ${base}/DeepAlu/DeepAlu_model/batch_cdgc/deepalu_${output}_predict.txt -s ${base}/DeepAlu/data_cluster/split_softclipped_sort_$ran_num -r $REF  -q $quick_model
...
python model_test_refine.py -i $input_gt -o ${base}/DeepAlu/DeepAlu_model/batch_cdgc/deepalu_${output}_predict.txt -s ${base}/DeepAlu/data_cluster/split_softclipped_sort_$ran_num -r $REF  -q $quick_model -t $threshold_model

rm multi_process/${output}.txt

bash transPredictTobed.sh batch_cdgc/deepalu_${output}_predict.txt batch_cdgc/deepalu_${output}.bed $threshold_model
...
#	fi	
	...
	echo "${base}/DeepAlu/data_cluster/split_softclipped_sort_$ran_num"
	echo "${base}/DeepAlu/DeepAlu_model/batch_cdgc/deepalu_${output}_predict.txt"
	echo "$input_gt"
fi
cd ${base}/DeepAlu/data_cluster
...
#bedtools getfasta -tab -fi $REF -bed <(cat ${base}/DeepAlu/final_vcf/batch_cdgc/deepalu_${output}.bed |perl -F'\t' -alne 'print "$F[0]:$F[1]\-$F[2]";'|xargs -n 1 -I {} -P 30  perl indel_len_filter.pl {} $bam_file ) |grep -Ev "TTTTTTTTTTTTTTTT|AAAAAAAAAAAAAAAA" |cut -f1|perl -npe "s/[:\-]/\t/g" |while read line;do grep "$line" ${base}/DeepAlu/final_vcf/batch_cdgc/deepalu_${output}.bed ;done |perl -F'\t' -alne '$pos=int(($F[1]+$F[2])/2);print "$F[0]\t$pos\t$F[3]\t$F[4]\t$F[5]";' > ${base}/DeepAlu/final_vcf/batch_cdgc/deepalu_${output}.vcf.bed

cat ${base}/DeepAlu/final_vcf/batch_cdgc/deepalu_${output}.bed  |perl -F'\t' -alne '$pos=int(($F[1]+$F[2])/2);print "$F[0]\t$pos\t0\tHG002\tME";' >input_${ran_num}.bed
bash order_RE_sample.sh $bam_file $ran_num input_${ran_num}.bed $ME_REF $REF
#1       29321412        29321512        1       63.76242115     ALU
#bedtools intersect -a ${base}/DeepAlu/final_vcf/batch_cdgc/deepalu_${output}.bed   -b <(ls split_softclipped_sort_$ran_num/|perl -npe "s/_map.*//"|sort |uniq |while read file;do has_indel=`cat split_softclipped_sort_$ran_num/${file}* |grep -v "^@"|perl indel_len_filter.pl $file`;echo -e "$file\t$has_indel" ;done |grep "0$"|perl -npe "s/.*HG002_//;s/[:\-]/\t/g;s/\.bam//" ) -wa > ${base}/DeepAlu/final_vcf/batch_cdgc/deepalu_${output}.vcf.bed
#cat ${base}/DeepAlu/final_vcf/batch_cdgc/deepalu_${output}.bed  |while read info;do has_indel=`perl indel_len_filter.pl $file`;echo -e "$file\t$has_indel" ;done |grep "0$"|perl -npe "s/.*HG002_//;s/[:\-]/\t/g;s/\.bam//" ) -wa > ${base}/DeepAlu/final_vcf/batch_cdgc/deepalu_${output}.vcf.bed
#bedtools intersect -a ${base}/DeepAlu/final_vcf/batch_cdgc/deepalu_${output}.bed  -b <(cat input_${ran_num}.bed|perl -npe "s/\t/:/g"|while read info;do has_indel=`perl indel_len_filter.pl $info`;echo -e "$file\t$has_indel" ;done |grep "0$"|perl -npe "s/.*HG002_//;s/[:\-]/\t/g;s/\.bam//" ) -wa > ${base}/DeepAlu/final_vcf/batch_cdgc/deepalu_${output}.vcf.bed
bedtools intersect -a ${base}/DeepAlu/final_vcf/batch_cdgc/deepalu_${output}.bed -b <(cat ${base}/DeepAlu/final_vcf/batch_cdgc/deepalu_${output}.bed |cut -f1-3|perl -npe "s/\t/:/;s/\t/\-/;" |xargs -n 1 -P 20 -I {} perl indel_len_filter.pl $bam_file $REF {}) -wa > ${base}/DeepAlu/final_vcf/batch_cdgc/deepalu_${output}.vcf.bed

rm input_${ran_num}.bed

cd split_softclipped_sort_$ran_num

ls |grep "_map"|perl -npe "s/_map.*//"|sort |uniq |while read file;do  cat <(../../DeepAlu_script/SE-MEI/extractSoftclipped -l 15 <(cat <(samtools view -H -T $REF $bam_file) <(cat ${file}_mapClip*|grep -v "^@")))|perl -npe "s/^(\+.*)\n/\1\t/"|perl -npe "s/^\+\t.*(\n)?//;s/\@soft\|.*/>${file}/"|sort |uniq |perl ../join_clip.pl|grep -v "^$" ;done  >tmp_clip.fa &
...
bedtools intersect -a ${base}/DeepAlu/final_vcf/batch_cdgc/deepalu_${output}.vcf.bed -b  <(cat tmp_clip.fa.out |grep -E "SINE\/Alu|LINE\/L1|Retroposon\/SVA"|perl -npe "s/^ +//;s/ +/\t/g"|cut -f5|cut -d'_' -f1-3|perl -npe "s/HG002_//"|cut -d'_' -f1|perl -npe "s/[:\-]/\t/g"|sort |uniq )  -wa |sort |uniq > ${base}/DeepAlu/final_vcf/batch_cdgc/deepalu_${output}.filtered.vcf.bed
#echo "1"
...
bedtools intersect -a ${base}/DeepAlu/final_vcf/batch_cdgc/deepalu_${output}.filtered.vcf.bed -b  tmp_polyN.bed  -v |sort |uniq > ${base}/DeepAlu/final_vcf/batch_cdgc/deepalu_${output}.polyN.vcf.bed
...
samtools index batch_cdgc/deepalu_${output}.bam
