...
if($polyAT_direction!=-1)
{
	...
	close T_FASTA_short;
	print "left 1\n";
	`RepeatMasker -q -no_is  -alu -species human -a -e rmblast -excln  tmp_left_long.fa`;
	print "left 2\n";
	`cat tmp_left_long.fa.out |perl -npe "s/ +/\t/g;s/^\t//"|cut -f5|tail --lines=+4 |sort |uniq >tmp_left_long_select.txt`;
	print "left 3\n";
#	`rm tmp_left_long.fa.*`;
...
if($polyAT_direction!=1)
{
	...
	close T_FASTA_short;
	print "right1\n";
	`RepeatMasker -q -no_is  -alu -species human -a -e rmblast -excln  tmp_right_long.fa`;
	print "right2\n";
	`cat tmp_right_long.fa.out |perl -npe "s/ +/\t/g;s/^\t//"|cut -f5|tail --lines=+4 |sort |uniq >tmp_right_long_select.txt`;
	print "right3\n";
	`rm tmp_left_long.fa.*`;
