...
ME_REF=${base}/DeepAlu/DeepAlu_model/reference/ME_add_ALU.fa
...
ls |grep "_map"|perl -npe "s/_map.*//"|sort |uniq |while read file;do  cat <(../../DeepAlu_script/SE-MEI/extractSoftclipped -l 15 <(cat <(samtools view -H $bam_file) <(cat ${file}_mapClip*|grep -v "^@")))|perl -npe "s/^(\+.*)\n/\1\t/"|perl -npe "s/^\+\t.*(\n)?//;s/\@soft\|.*/>${file}/"|sort |uniq |perl ../join_clip.pl|grep -v "^$" ;done  >tmp_clip.fa
...
then
	cp ${base}/DeepAlu/final_vcf/batch_cdgc/deepalu_${output}.filtered.vcf.bed  ${base}/DeepAlu/final_vcf/batch_cdgc/deepalu_${output}.final.vcf.bed
else
	bedtools intersect -a ${base}/DeepAlu/final_vcf/batch_cdgc/deepalu_${output}.filtered.vcf.bed -b  simple_repeat.bed  -v |sort |uniq > ${base}/DeepAlu/final_vcf/batch_cdgc/deepalu_${output}.final.vcf.bed
fi
#echo "4"
